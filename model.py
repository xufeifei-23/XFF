...
class Model(object):
    def __init__(self, config):
        ...
        length = tf.reduce_sum(used, reduction_indices=1)  #求出每个句子的真是长度
        self.lengths = tf.cast(length, tf.int32)
        self.batch_size = tf.shape(self.word_inputs)[0]
        self.num_setps = tf.shape(self.word_inputs)[-1]

        # embedding层单词和分词信息
        embedding = self.embedding_layer(self.word_inputs, self.seg_inputs, config)  #完成特征的嵌入

        # lstm输入层
        lstm_inputs = tf.nn.dropout(embedding, self.dropout)   #将输入的特征随机的失活
        ...
        # 损失
        self.loss = self.crf_loss_layer(self.logits, self.lengths)    #做条件随机场损失
        ...
        with tf.variable_scope('optimizer'):
            ...
            # g是梯度的倒数，v是相对应的那个tensor
            self.train_op = self.opt.apply_gradients(capped_grad_vars, self.global_step)   #实现梯度下降
    ...
    def embedding_layer(self, word_inputs, seg_inputs, config, name=None):
        ...
        with tf.variable_scope("word_embedding" if not name else name), tf.device('/cpu:0'):
            ...
            embedding.append(tf.nn.embedding_lookup(self.word_lookup, word_inputs)) #结果是三维的
            ...
            embed = tf.concat(embedding, axis=-1)   #（最后是none，none，120）
        return embed
    ...
    def biLSTM_layer(self, lstm_inputs, lstm_dim, lengths,name=None):
        ...
        return tf.concat(outputs, axis=2)     #讲一个前向和后向的输出拼接起来，形成（none，none，200）
    ...
    def project_layer(self, lstm_outputs, name=None):
        ...
        with tf.variable_scope('project_layer' if not name else name):
            ...
            with tf.variable_scope('hidden_layer'):
                ...
                out_put = tf.reshape(lstm_outputs, shape=[-1, self.lstm_dim*2])  #只能做两维的运算
        ...
        return tf.reshape(pred, [-1, self.num_setps, self.num_tags])  #再映射回来，这里最后一程没有做softmax
