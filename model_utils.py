...
def create(sess, Model, ckpt_path, load_word2vec, config, id_to_word, logger):
    ...
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
        ...
        logger.info("重新训练模型")
        sess.run(tf.global_variables_initializer())  #实现了把所有变量都初始化
        if config['pre_emb']:  #实现预训练词向量
            ...
            sess.run(model.word_lookup.assign(emb_weights))   #把修改好的值再赋值回去  这个在后面训练的时候还会再修改的
            logger.info("加载词向量成功")
    return model
