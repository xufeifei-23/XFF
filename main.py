...
flags.DEFINE_string('train_file', os.path.join('data', 'example3.train'), '训练数据路径')
flags.DEFINE_string('dev_file', os.path.join('data', 'example3.dev'), '校验数据路径')
flags.DEFINE_string('test_file', os.path.join('data', 'example3.test'), '测试数据路径')
...
def train():
    ...
    tf_config = tf.ConfigProto()  #配置gpu
    tf_config.gpu_options.allow_growth = True
    steps_per_epoch =train_manager.len_data   #每一个轮次有多少步，也就是每一个轮次需要把所有的批次都跑完
